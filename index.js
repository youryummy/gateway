...
const netId = process.env.IP_RANGE ?? info[0].dst;
...
async function updateRoutes() {
    ...
    exec(`nmap -sn -n -oG - --send-ip ${netId} | awk '/Status: Up/ {print $2}'`).then(({stdout, _stderr}) => {
        ...
        Promise.allSettled(
            hosts.map(async host => {
                let res = await fetch(`http://${host}/docs/swagger.json`);
                if(res.ok) {
            ...
            let entries = res.filter(prom => prom.status === "fulfilled" && prom.value).map(prom => prom.value);
